...
public class HiloServidor extends Thread {
		...
		try {
			...
			while (true) {
				...
				if (recibido.mensaje.contains("salir")) {
					// SI EL CLIENTE QUIERE SALIR CIERRO LOS SOCKETS Y EL CANAL DE COMUNICACIÓN Y ME
					// SALGO DEL BUCLE.
					entrada.close();
					salida.close();
					socket.close();
	...
	public void enviarElMensaje() throws IOException {
		...
		if (!existe) {
			...
			ms.add(mensaj);
			// GUARDO EL MENSAJE CON EL DESTINATARIO EN EL HASHMAP Y EL ARRAY DE ESE
			// MENSAJES DE ESE DESTINATARIO
